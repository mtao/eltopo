...
EdgeFlipper::PreEdgeFlipInfo::PreEdgeFlipInfo( size_t edge, size_t tri0, size_t tri1 ) :
    m_edge(edge),
    m_tri0(tri0),
    m_tri1(tri1)
{}
...
EdgeFlipper::PostEdgeFlipInfo::PostEdgeFlipInfo(const PreEdgeFlipInfo& pre_split_info, 
                                                size_t new_triangle_index_0, 
                                                size_t new_triangle_index_1 ) :
    m_pre_flip_info(pre_split_info),
    m_new_triangle_index_0(new_triangle_index_0),
    m_new_triangle_index_1(new_triangle_index_1)
{}
...
size_t EdgeFlipper::vertex_valence( size_t vertex_index )
{
    return m_surf.m_mesh.m_vertex_to_edge_map[ vertex_index ].size();
}

int EdgeFlipper::total_mesh_regularity()
{
    int R = 0;
    for ( size_t i = 0; i < m_surf.get_num_vertices(); ++i )
    {
        ...
        R += ( static_cast<int>(vertex_valence(i)) - 6 ) * ( static_cast<int>(vertex_valence(i)) - 6 );
    }
...
bool EdgeFlipper::regularity_improves( size_t edge, 
                                      size_t , 
                                      size_t , 
                                      ...
                                      size_t third_vertex_1 )
{
    ...
    err = vertex_valence(third_vertex_0) - 6;
    R += err * err;
    ...
    err = vertex_valence(b) - 1 - 6;
    R_after += err * err;
    
    err = vertex_valence(third_vertex_0) + 1 - 6;
    R_after += err * err;
    
    err = vertex_valence(third_vertex_1) + 1 - 6;
    R_after += err * err;
...
void EdgeFlipper::add_observer( EdgeFlipObserver* observer )
{
