...
EdgeCollapser::PreEdgeCollapseInfo::PreEdgeCollapseInfo( size_t edge, size_t vertex_to_keep, size_t vertex_to_delete, const Vec3d& vertex_new_position ) :
    m_edge( edge ),
    m_vertex_to_keep( vertex_to_keep ),
    m_vertex_to_delete( vertex_to_delete ),
    m_vertex_new_position( vertex_new_position )
{}
...
EdgeCollapser::PostEdgeCollapseInfo::PostEdgeCollapseInfo( const PreEdgeCollapseInfo& pre_collapse_info,
    ...
    m_pre_collapse_info(pre_collapse_info),
    m_triangles_incident_to_edge(triangles_incident_to_edge),
    m_triangles_incident_to_deleted_vertex(triangles_incident_to_deleted_vertex),
    m_new_triangles(new_triangles),
    m_edges_incident_to_deleted_vertex( edges_incident_to_deleted_vertex )
{}
                                         ...
                                         std::vector<size_t>& moving_triangles )
{
    ...
    //      {
    //         moving_triangles.erase( moving_triangles.begin() + i );
    //         --i;
    //      }
    //   }   
                                     ...
                                     std::vector<size_t>& moving_edges )
{
    ...
    // Disregard collapsed edge
    
    //   for ( size_t i = 0; i < moving_edges.size(); ++i )
    //   {
    //      if ( moving_edges[i] == edge_index )
    //      {
    //         moving_edges.erase( moving_edges.begin() + i );
    //         --i;
    //      }
    //   }
    //   
    //   std::sort( moving_edges.begin(), moving_edges.end() );
    //   std::vector<size_t>::iterator new_end = std::unique(moving_edges.begin(), moving_edges.end());
    //   moving_edges.erase(new_end, moving_edges.end());
                                                                     ...
                                                                     const Vec3d& )
{
    ...
    std::vector<size_t> moving_triangles;
    get_moving_triangles( source_vertex, destination_vertex, edge_index, moving_triangles );
...
#define USE_VERTEX_RANKS
    ...
    unsigned int keep_rank = m_surf.classify_vertex( vertex_to_keep );
    unsigned int delete_rank = m_surf.classify_vertex( vertex_to_delete );
    ...
    if ( mag ( m_surf.get_position(m_surf.m_mesh.m_edges[edge][1]) - m_surf.get_position(m_surf.m_mesh.m_edges[edge][0]) ) > 0 )
    {
        ...
        bool bad_angle = collapse_edge_introduces_bad_angle( vertex_to_delete, vertex_to_keep, edge, vertex_new_position );
    ...
    std::vector<size_t> edges_incident_to_deleted_vertex = m_surf.m_mesh.m_vertex_to_edge_map[vertex_to_delete];
    
    PreEdgeCollapseInfo pre_info( edge, vertex_to_keep, vertex_to_delete, vertex_new_position );
    ...
    for ( size_t i=0; i < triangles_incident_to_vertex.size(); ++i )
    {
        ...
        new_triangles.push_back( new_triangle_index );
    }
    ...
    PostEdgeCollapseInfo post_info( pre_info, triangles_incident_to_edge, triangles_incident_to_vertex, new_triangles, edges_incident_to_deleted_vertex );
...
void EdgeCollapser::process_mesh()
{
    ...
    while ( collapse_occurred )
    {
        ...
        {
            size_t e = sortable_edges_to_try[si].edge_index;
            ...
            {        
                if ( m_surf.m_verbose )
                {
                    printf( "collapsing edge %d / %d, length = %f\n", (int)e, (int)m_surf.m_mesh.m_edges.size(), edge_length );
                    printf( "edge %d %d... ", (int)m_surf.m_mesh.m_edges[e][0], (int)m_surf.m_mesh.m_edges[e][1] );
                }
...
void EdgeCollapser::add_observer( EdgeCollapseObserver* observer )
{
